...
## Interactive Charts – Assignment 36 (Plotly)

The four charts below are generated by `assignment-36-interactive-charts.py` and saved as
self-contained HTML files in `output/`.  Open any `.html` file in a browser — no Python or
server required.

### Chart 1 – Daily Revenue & Profit Trend
- **File:** `chart1_revenue_trend_interactive.html`
- **Chart Type:** Multi-series line chart with range selector + range slider
- **Interactive Features:**
  - **Unified hover tooltip** — date, revenue ($), and profit ($) in one panel
  - **Date range buttons** — jump to 1M, 3M, 6M, YTD, or All
  - **Range slider** — drag endpoints to select any custom date window
  - **Zoom / Pan** — click-drag to zoom; double-click to reset

---

### Chart 2 – Product-Line Metrics with Dropdown
- **File:** `chart2_product_dropdown_interactive.html`
- **Chart Type:** Horizontal bar chart with `updatemenus` dropdown
- **Interactive Features:**
  - **Dropdown filter** — switch instantly between Revenue, Profit, and Orders
  - **Multi-field hover** — all three metrics visible in the tooltip regardless of active view
  - **Zoom / Pan** — explore data at any granularity

---

### Chart 3 – Marketing Spend vs Revenue Scatter
- **File:** `chart3_marketing_scatter_interactive.html`
- **Chart Type:** Scatter plot with OLS trendline
- **Interactive Features:**
  - **Color-coded channels** — Email, Social, Paid Search, Webinar toggleable via legend click
  - **Outlier annotation** — hover reveals "Low ROI!" label for the anomalous campaign
  - **Zoom / Pan** — click-drag to drill into clusters; Shift+drag to pan

---

### Chart 4 – Multi-KPI Linked Dashboard (Subplots)
- **File:** `chart4_kpi_subplots_interactive.html`
- **Chart Type:** 2×2 linked subplot grid (revenue, profit, orders, margin %)
- **Interactive Features:**
  - **Linked x-axes** — zooming any panel syncs the date range across all four panels
  - **Unified hover** — one hover event surfaces all four metrics simultaneously
  - **Date range buttons** — shared 1M / 3M / 6M / All selector

---

### Running as a Streamlit Dashboard

```bash
pip install plotly streamlit
streamlit run assignment-36-interactive-charts.py
```

The dashboard embeds all four Plotly charts via `st.plotly_chart(fig, use_container_width=True)`,
preserving every hover, zoom, and filter interaction inside the Streamlit app.
